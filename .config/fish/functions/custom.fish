...
function mef
    curl -s "$1" | pup "a#downloadButton attr{href}" | xargs wget 
end
...
function up
    curl -F "filee=@$1" "https://cloud-shell.onrender.com" 
end
...
function short
    curl -s http://tinyurl.com/api-create.php?url="$1" && echo " "; 
end
...
# tor
function tsrc
   /usr/bin/zsh -c "if [[ $1 -eq 1 ]]; then . torsocks on; else . torsocks off; fi "
end
